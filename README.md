...
- `POST /api/discussions/recommend`：根据讨论问题和用户选中的立场召回知乎内容，并分为相近、不同和立场不明三组。
...
| `/space/:momentId` | 讨论空间：承接当前 AI 讨论、投票选择与知乎真实表达 |

收藏页会把用户选中的真实内容带入阅读页，并保留标题、作者、摘要、互动数据和知乎原文入口；未登录时也可通过「探索知乎」调用服务端搜索公开真实内容后导入。阅读页会优先调用服务端 AI 生成 3～6 个讨论瞬间，并按当前讨论检索知乎真实相关内容。用户提交投票后，服务端会按其具体选择对候选内容进行立场分类与相关度排序，分别展示「相近观点」和「不同观点」；证据不足的内容会单独标记为立场不明。AI 使用低推理强度并设置 30 秒上限，文章分析成功结果会在浏览器缓存 24 小时。服务端密钥尚未配置、超时或接口失败时，页面会保留投票前的主题相关内容并显示错误原因。
